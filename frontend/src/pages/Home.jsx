...
function Home() {
  ...
  const [isAssistantActive, setIsAssistantActive] = useState(false) // Track continuous mode activation
      ...
      const utterance = voicePersonality.createUtterance(speakText, {
        ...
        onError: (error) => {
          ...
          if (error.error === 'not-allowed') {
            ...
            // Still show the text response so user can read it
            setAiText(speakText);
          ...
          // Restart voice assistant after error with longer delay to avoid interruption
          setTimeout(() => {
  ...
  // Helper function to ensure voice assistant restarts after actions
  ...
  const handleCommand = async (data) => {
    ...
    if (type === 'google-search' && url) {
      ...
      ensureListeningAfterAction(2000); // Restart listening after opening URL
    }
    ...
    if (action === 'app-launch') {
      ...
      ensureListeningAfterAction(2000); // Restart listening after app launch
      return;
    }
    ...
    if (action === 'whatsapp-send' || type === 'whatsapp-send') {
      ...
      if (type === 'whatsapp-send') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after WhatsApp
        return;
      }
      ...
      if (type === 'telegram-send') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after Telegram
        return;
      }
      ...
      if (action === 'screen-record') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after screen recording
        return;
      }
      ...
      if (type === 'screenshot') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after screenshot
        return;
      }
      ...
      if (action === 'screen-share') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after screen sharing
        return;
      }
      ...
      if (action === 'instagram-dm') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after Instagram DM
        return;
      }
      ...
      if (action === 'instagram-story') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after Instagram story
        return;
      }
      ...
      if (action === 'instagram-profile') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after Instagram profile
        return;
      }
      ...
      if (action === 'cast-media') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after casting
        return;
      }
      ...
      if (action === 'cast-youtube') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after casting YouTube
        return;
      }
      ...
      if (action === 'camera-photo') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after camera action
        return;
      }
      ...
      if (action === 'camera-video') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after video recording
        return;
      }
      ...
      if (action === 'pick-contact') {
        ...
        ensureListeningAfterAction(2000); // Restart listening after picking contacts
        return;
      }
    }
    ...
    useEffect(() => {
      ...
      // Handle partial transcripts (real-time streaming)
      ...
      // Initialize VAD Service for interrupt detection
    ...
    }, [userData, serverUrl]); // Only re-run when userData or serverUrl changes
    ...
    return (
      <div className='w-full h-[100vh] bg-gradient-to-t from-[black] to-[#02023d] flex justify-center items-center flex-col gap-[15px] overflow-hidden relative'>
        ...
        {/* Partial Transcript Display (Real-time streaming) */}
